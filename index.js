...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀:
/* 
The nested function nestedFunction() is able to access 'internal' from myFunction() because of closures.
A closure is when a nested function is able to move up one level in scope to the higher level function and grab a variable/value
that it does not have within its own scope.  In this case, nestedFunction() does not have the variable 'internal' in its own scope,
so it will look in the next level up for the variable.  It is able to find it in the higher level function myFunction() and use the value.
*/
...
function lowerCaseNames(zooAnimals) {
  const newArr = zooAnimals.map((animal) => animal.animal_name.toLowerCase());
  return newArr;
}
...
function lowPopulationAnimals(zooAnimals) {
  const newArr = zooAnimals.filter((animal) => animal.population < 5);
  return newArr;
}
...
function USApop(zooAnimals) {
  const USApop = zooAnimals.reduce((acc, cur) => {
